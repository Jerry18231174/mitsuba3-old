...
public:
    ...
    Spectrum eval_specular_reflectance(const SurfaceInteraction3f &si_,
                                      Mask active) const override {
        SurfaceInteraction3f si(si_);

        if (m_brdf[0] == m_brdf[1]) {
            si.wi.z() = dr::abs(si.wi.z());
            return m_brdf[0]->eval_specular_reflectance(si, active);
        } else {
            Spectrum result = 0.f;
            Mask front_side = Frame3f::cos_theta(si.wi) > 0.f && active,
                 back_side  = Frame3f::cos_theta(si.wi) < 0.f && active;
            ...
            if (dr::any_or<true>(front_side))
                result = m_brdf[0]->eval_specular_reflectance(si, front_side);

            if (dr::any_or<true>(back_side)) {
                si.wi.z() *= -1.f;
                dr::masked(result, back_side) =
                    m_brdf[1]->eval_specular_reflectance(si, back_side);
            }
